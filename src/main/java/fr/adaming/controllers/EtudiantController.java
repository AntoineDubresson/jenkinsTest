...
public class EtudiantController {
	...
	public String soumettreSupprimer(RedirectAttributes rda, @RequestParam("pId") int idIn) {
		...
		if (verif) {
			...
			// L'objet redirectattributes sert a transporter les attributs du model mvc lors
			// de la redirection
	...
	public String afficheRechercher() {
		return "recherche";
	}

	@RequestMapping(value = "/submitGet", method = RequestMethod.POST)
	public String soumettreRechercher(RedirectAttributes rda, @RequestParam("pId") int idIn) {
		Etudiant eIn = new Etudiant();
		eIn.setId(idIn);
		Etudiant eOut = etuService.getEtudiantById(eIn, this.formateur);
		if (eOut != null) {
	...
	public String getModifLien(Model modele, RedirectAttributes rda, @RequestParam("pId") int idIn) {
		Etudiant eIn = new Etudiant();
		eIn.setId(idIn);
